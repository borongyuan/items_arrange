...
int main()
{
	...
	char*		rawfile = "C:\\workspace\\data\\eegs\\rawdata.csv";
	char*		fftfile = "C:\\workspace\\data\\eegs\\fftdata.csv";
	char*       bpnfile = "C:\\workspace\\data\\eegs\\bodyposition.csv";
	char*       bmvfile = "C:\\workspace\\data\\eegs\\bodymove.csv";

	int *score;

	unsigned char* working_space = swd_initWorkingSpace();
	...
	while(1)
	{
		swd_RealTimeHalfMinute(&rawdata[SECONDS_OF_EPOCH*NFFT*numEpoch], &fftdata2[SECONDS_OF_EPOCH*NFFT*numEpoch], working_space);
	...
	swd_algorithm(bodyposition, bodymove, style_totalSecond, working_space);
    
    score = getTotalHalfMinute(working_space);
