...
static void thSleepWake(float *ratio, int length, float *th, float gain)
{
	int i;
	const int window = 5;
	const int startPoint = STARTPOINT;
...
int get_swd_WorkingSpaceSize(){
	return outaasm_size_of_working_space();
}
...
void enhancement(int *score, int len)
{
	int i = 0;

	while(1)
	{
		if(score[i]==WAKE&&score[i+1]==DEEP)
		{
			score[i+1] = LIGHT;
			i = 0;
			continue;
		}
		if(score[i]==DEEP&&score[i+1]==REM)
		{
			score[i+1] = LIGHT;
			i = 0;
			continue;
		}
		if(score[i]==REM&&score[i+1]==DEEP)
		{
			score[i+1] = LIGHT;
			i = 0;
			continue;
		}
		if(score[i]==WAKE&&score[i+1]==REM)
		{
			score[i+1] = WAKE;
			i = 0;
			continue;
		}
		if(score[i]==DEEP&&score[i+1]==WAKE)
		{
			score[i+1] = LIGHT;
			i = 0;
			continue;
...
void enhancement2(int *score, int len)
{
	int i = 0;
...
{																				
	 ...
	 enhancement2(score, raw_thirty_seconds_len);
