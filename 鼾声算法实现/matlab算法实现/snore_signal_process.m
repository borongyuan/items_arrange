...
audio_data=double(audio_data(:,1));
% i=find(audio_data<0);
% audio_data(i)=2^16-(abs(audio_data(i)));
...
%              'CutoffFrequency1', 0.01, 'CutoffFrequency2', 5000,... %带通滤波器
%              'SampleRate', sample_frequent);
% for i=1:size(index_matrix,1)
%    index_matrix(i,:)=filter(bpFilt,index_matrix(i,:));
% end
 
 %% 时域特征分析
 short_time_energy=sum(index_matrix.^2,2); %短时能量
 short_time_max_amplify=max(abs(index_matrix),[],2);%信号短时最大振幅值
 short_time_energy_db=max(10*log10(short_time_energy/window_size),0); %短时平均能量分贝数
 short_time_max_amplify_db=max(20*log10(short_time_max_amplify),0); %短时最大振幅分贝数
...
%  end
 ...
 for i=1:size(index_matrix,1)                                                                             %mfcc特征
    short_time_mfcc_feature(i,:)=mfcc_feature_achieve(index_matrix(i,:).',24,256,sample_frequent,0,0.5,12);
 end
...
%  figure('name','短时最大振幅分贝数'),plot(short_time_max_amplify_db); %画短时最大振幅分贝数图
...
% figure('name','频谱图'),imagesc(index_matrix_fft(:,2:end)); %画能量谱图
...
%  figure('name','mfcc倒频谱色彩图'),imagesc(short_time_mfcc_feature); colorbar%画mfcc倒频谱图
%  figure('name','mfcc倒频谱曲线图'),plot(short_time_mfcc_feature(:,1)); colorbar%画mfcc倒频谱图
%  n=0:(window_size-1);
